...
crates/
  core/                 # 统一工具层:文本域(&str→String)+ 字节域(&[u8]→Vec<u8>)
    src/
      registry.rs       # Tool trait + ToolMeta/ParamSpec/ToolArgs + tools()/find_tool()(109 文本工具)
      ...
      fileconv/          # 字节域
        archive.rs image.rs pdf.rs engine.rs fs_util.rs path.rs route.rs  mod.rs
...
src/                     # Svelte 5 前端
  ...
  components/            # TopBar · Sidebar · ToolWorkspace · SharedInput · OutputPanel · ParamBar · ModeTabs · EmptyState · CommandPalette · EngineManager
  bindings.ts · main.ts
docs/                    # prd · tech · api · flow · todo
```
...
- **GUI 动态渲染**:`list_tools()` + `run_tool(id, input, args)` 通用入口,前端 mount 时拉取元数据动态渲染;`output_kind` 驱动高亮/SVG,不用 `id` 前缀推断。
