...
| Generators | hash、uuid、password、lorem、qr | hash 含 MD5/SHA1/SHA256/SHA512;uuid v4/v7;qr 生成 PNG/SVG |
| Text | case、sort-dedup、reverse、regex、diff | case 含 snake/camel/kebab/title;diff 文本逐行 |
| Crypto | aes-gcm、rsa、kdf、hmac | AES-GCM 加解密;RSA 密钥生成/加解密/签名;kdf 含 PBKDF2/Argon2 |
| Net/Time | ipcalc、timestamp、cron、http、dns | IP/子网计算;时间戳互转;cron 解析下次触发;HTTP 探测;DNS 查询 |

## 用户故事与验收(代表性,Given/When/Then)

### base64

- Given 输入 `Hello`,When `nextool base64 encode "Hello"`,Then 输出 `SGVsbG8=`。
- Given 输入 `SGVsbG8=`,When `nextool base64 decode`,Then 输出 `Hello`。
- Given 非法 Base64 `!!!!`,When decode,Then 非零退出码 + 错误信息到 stderr。

### json-yaml

- Given JSON `{"a":1,"b":[2,3]}`,When `nextool json-yaml to-yaml`,Then 输出合法 YAML。
- Given 非法 JSON,When 转换,Then 非零退出码 + 指出错误位置。

### regex

- Given 模式 `\d+` 与文本 `a12b3`,When `nextool regex match`,Then 输出 `12`、`3` 各一行。
- Given 模式 `\d+` 与替换 `#$0`,When `nextool regex replace`,Then 输出 `a#12b#3`。

### diff

- Given 两段文本,When `nextool diff`,Then 输出 unified diff,新增 `+`、删除 `-`。

### aes-gcm

- Given 明文与口令,When `nextool aes-gcm encrypt`,Then 输出密文(含 nonce);When decrypt 同口令,Then 还原明文。
- Given 错误口令,When decrypt,Then 非零退出码(认证失败)。

### ipcalc

- Given `192.168.1.5/24`,When `nextool ipcalc`,Then 输出网络地址、广播、掩码、可用主机范围。

### timestamp

- Given Unix 时间戳 `1700000000`,When `nextool timestamp to-human --tz Asia/Shanghai`,Then 输出对应时区可读时间。
