...
enum TextCmd {
    ...
    Case { mode: CaseModeArg, input: Option<String> },
    /// 行排序(升序)
    ...
    /// 正则匹配:每匹配一行输出
    RegexMatch { pattern: String, input: Option<String> },
    /// 正则替换:支持 $0/$1 捕获组
    RegexReplace { pattern: String, replacement: String, input: Option<String> },
    /// 文本 Diff:比较两个输入(unified diff)
    Diff { a: Option<String>, b: Option<String> },
}
...
pub fn run(args: TextArgs) -> Result<(), String> {
    match args.cmd {
        TextCmd::Case { mode, input } => {
            ...
            println!("{}", nextool_core::case_convert(&input, mode.into()).map_err(|e| e.to_string())?);
        }
        TextCmd::SortLines { input } => {
            println!("{}", nextool_core::sort_lines(&read_input(input)?).map_err(|e| e.to_string())?);
        }
        TextCmd::DedupLines { input } => {
            println!("{}", nextool_core::dedup_lines(&read_input(input)?).map_err(|e| e.to_string())?);
        }
        TextCmd::Reverse { input } => {
            println!("{}", nextool_core::reverse_text(&read_input(input)?).map_err(|e| e.to_string())?);
        }
        TextCmd::RegexMatch { pattern, input } => {
            let input = read_input(input)?;
            println!("{}", nextool_core::regex_match(&pattern, &input).map_err(|e| e.to_string())?);
        }
        TextCmd::RegexReplace { pattern, replacement, input } => {
            let input = read_input(input)?;
            println!(
                "{}",
                nextool_core::regex_replace(&pattern, &replacement, &input).map_err(|e| e.to_string())?
            );
        }
        TextCmd::Diff { a, b } => {
            let a = read_input(a)?;
